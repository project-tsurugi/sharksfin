...
struct Options {
    ...
    static Options parse(std::vector<char*> const& args) {
        Options result;
        if (args.size() <= 2U) {
            std::cerr << "usage: " << args[0] << " [options...] <command-name> <command-options...>" << std::endl;
        ...
        for (std::size_t i = 1, n = args.size(); i < n; ++i) {
            ...
            for (auto& spec : command_list) {
                if (s == spec.name) {
                    if (i + 1 + spec.arguments.size() > args.size()) {
                        std::cerr << "usage: " << args[0] << " [options...] " << spec.name;
                    ...
                    for (std::size_t j = 0, n = spec.arguments.size(); j < n; ++j) {
                        command.arguments.emplace_back(args[i + 1 + j]);
                    }
