...
#define _LOCATION_PREFIX_TO_STREAM(stream)  \
    if (constexpr auto __tmplp = sharksfin::shirakami::location_prefix(__PRETTY_FUNCTION__, __FUNCTION__); false) {} else \
    if (constexpr auto __tmplplen = std::string_view{__tmplp.data()}.length(); false) {} else \
    if (static constexpr auto __tmplptrim = sharksfin::shirakami::shrink_array<__tmplplen>(__tmplp); false) {} else \
